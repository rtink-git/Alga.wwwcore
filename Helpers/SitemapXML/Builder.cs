...
{
    /// <summary>
    /// Что такое Sitemap? https://sitemaps.org/protocol.html
    /// </summary>
    public static class Builder
    {
        ...
        public static MemoryStream GenerateXmlStream(ReqUrlEnumerable req)
        {
            // Строим элементы URL
            ...
            // Создаём MemoryStream для записи данных
            ...
            var settings = new XmlWriterSettings
            {
                ...
                Indent = true,
                OmitXmlDeclaration = false // XML декларация будет добавлена автоматически
            };

            using (var writer = XmlWriter.Create(stream, settings))
            {
                // Записываем XML в поток
                doc.WriteTo(writer);
                writer.Flush(); // Убедимся, что все данные записаны в поток
            }

            // Перемещаем указатель потока на начало, чтобы его можно было прочитать
