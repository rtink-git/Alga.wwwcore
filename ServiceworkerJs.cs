...
class ServiceworkerJs
{
  internal void Build(Models.Config config, FrozenDictionary<string, Models.SchemeJsonM>? schemes, FrozenDictionary<string, HashSet<string>>? modules)
  {
    try
    {
      ...
      string serviceWorkerPath = Path.Combine(wwwrootPath, $"serviceworker.{config?.CurrentVersion}.js");

      var filesToDelete = Directory.GetFiles(wwwrootPath, "*.js", SearchOption.TopDirectoryOnly).Where(f => Path.GetFileName(f).Contains("serviceworker"));
