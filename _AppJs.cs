...
window.loadPageContent = async function(path) {
  ...
  if (cacheResponses.has(cacheKey)) {
    return applyCachedResponse(cacheResponses.get(cacheKey));
  }
  ...
  try {
    ...
    console.error('Ошибка при обращении к кэшу:', err);
  }
...
function getElementKey(el) {
  ...
  if (el.tagName === 'STYLE') {
    return `STYLE:${el.textContent.length}`;
  }
...
async function replaceHead(newHead) {
  ...
  const isEqualElement = (oldEl, newEl) => {
    ...
    if (oldEl.tagName === 'META') {
      return oldEl.name === newEl.name && oldEl.content === newEl.content;
    }

    if (oldEl.tagName === 'LINK') {
      return oldEl.rel === newEl.rel && oldEl.href === newEl.href;
    }
