...
sealed class Builder
{
  ...
  {
    var filesToDelete = Directory.GetFiles(req.DirectoryPath, "*.js", SearchOption.TopDirectoryOnly).Where(f => Path.GetFileName(f).Contains("app"));
    foreach (var file in filesToDelete)
      File.Delete(file);

    var url = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", $"app.{req.Version}.js");
...
if (window.IS_RUNNING_IN_TELEGRAM_WEBAPP) {{
    const tg = window.Telegram.WebApp;
    tg.ready();
    tg.setBackgroundColor('{req.BackgroundColor}');
...
const registerServiceWorker = async () => {{
  ...
  try {{
    const registration = await navigator.serviceWorker.register('/serviceworker.{req.Version}.js');
